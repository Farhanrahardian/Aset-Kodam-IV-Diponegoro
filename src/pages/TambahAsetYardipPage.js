...
const kotaData = {
  jateng: [
    { id: "semarang", name: "Semarang", bounds: [[-6.95, 110.35], [-7.05, 110.45]] },
    { id: "solo", name: "Surakarta (Solo)", bounds: [[-7.52, 110.75], [-7.62, 110.85]] },
    { id: "yogya", name: "Yogyakarta", bounds: [[-7.72, 110.32], [-7.82, 110.42]] },
    { id: "magelang", name: "Magelang", bounds: [[-7.42, 110.18], [-7.52, 110.28]] },
    { id: "salatiga", name: "Salatiga", bounds: [[-7.32, 110.42], [-7.42, 110.52]] },
    { id: "tegal", name: "Tegal", bounds: [[-6.83, 109.12], [-6.93, 109.22]] },
    { id: "pekalongan", name: "Pekalongan", bounds: [[-6.83, 109.63], [-6.93, 109.73]] },
    { id: "purwokerto", name: "Purwokerto", bounds: [[-7.42, 109.22], [-7.52, 109.32]] },
    { id: "cilacap", name: "Cilacap", bounds: [[-7.68, 109.02], [-7.78, 109.12]] },
    { id: "kudus", name: "Kudus", bounds: [[-6.78, 110.82], [-6.88, 110.92]] },
    { id: "jepara", name: "Jepara", bounds: [[-6.55, 110.62], [-6.65, 110.72]] },
    { id: "rembang", name: "Rembang", bounds: [[-6.68, 111.32], [-6.78, 111.42]] }
  ],
  diy: [
    { id: "jogja", name: "Yogyakarta", bounds: [[-7.72, 110.32], [-7.82, 110.42]] },
    { id: "sleman", name: "Sleman", bounds: [[-7.62, 110.28], [-7.72, 110.38]] },
    { id: "bantul", name: "Bantul", bounds: [[-7.82, 110.28], [-7.92, 110.38]] },
    { id: "kulonprogo", name: "Kulon Progo", bounds: [[-7.72, 110.05], [-7.82, 110.15]] },
    { id: "gunungkidul", name: "Gunung Kidul", bounds: [[-7.92, 110.45], [-8.02, 110.55]] }
  ]
};
...
const TambahAsetYardipPage = () => {
  ...
  const handleProvinceChange = (e) => {
    ...
    if (province) {
      toast.success(`Provinsi ${province === 'jateng' ? 'Jawa Tengah' : 'DI Yogyakarta'} dipilih!`);
    }
  };
  ...
  const handleCityChange = (e) => {
    ...
    if (cityId && selectedProvince) {
      ...
      if (selectedCityData) {
        ...
        toast.loading(`Memuat peta untuk ${selectedCityData.name}...`, { id: 'map-loading' });
  ...
  const handleDrawingCreated = (data) => {
    ...
    toast.success(`✅ Lokasi berhasil digambar! Luas area: ${data.area?.toFixed(2)} m²`);
  };
  ...
  const handleSaveAsset = async (assetData) => {
    ...
    try {
      ...
      const selectedCityData = kotaData[selectedProvince].find(c => c.id === selectedCity);
      ...
      const payload = {
        ...
        provinsi: selectedProvince === 'jateng' ? 'Jawa Tengah' : 'DI Yogyakarta',
      ...
      const response = await axios.post(`${API_URL}/yarsip_assets`, payload);
      ...
      setError(`Gagal menyimpan aset yardip: ${err.response?.data?.message || err.message}`);
    }
  };
          ...
          <Card.Header className="bg-gradient" style={{background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white"}}>
            <div className="d-flex align-items-center">
              <div>
                ...
                <small className="opacity-75">Tentukan provinsi dan kota untuk penempatan aset Yardip</small>
              </div>
            </div>
          </Card.Header>
              ...
              <Col md={6}>
                <Form.Group className="mb-4">
                  ...
                  >
                    ...
                    {selectedProvince && kotaData[selectedProvince].map(city => (
              ...
              {selectedProvince && selectedCity && (
                <Alert variant="success" className="mb-0 border-0" style={{background: "rgba(25,135,84,0.1)"}}>
                  <div className="d-flex align-items-center">
                    ...
                    <div>
                      <strong>Siap!</strong> Lokasi telah dipilih. Peta akan segera dimuat dengan auto-zoom ke area {getSelectedCityData()?.name}.
                    </div>
                  </div>
                </Alert>
              )}
      ...
      {showMapAndForm && !isLoadingMap && (
        <>
              ...
              <div className="d-flex align-items-center">
                ...
                <div>
                  ...
                  <small className="text-success">
                    ...
                    Peta telah auto-zoom ke area target • 
                    Siap untuk menggambar aset
                  </small>
                </div>
              </div>
              <Button variant="outline-danger" size="sm" onClick={resetSelection}>
          ...
          {process.env.NODE_ENV === "development" && (
            <Alert variant="info" className="mb-3 border-0" style={{background: "rgba(13,202,240,0.05)"}}>
              <details>
                ...
                <div className="small">
                  <div className="row">
                    <div className="col-md-6">
                      ...
                      - Collection: yarsip_assets<br />
                      - Endpoint: POST /yarsip_assets<br />
                      - ID Prefix: Y (Yardip)<br />
                      - Selected Province: {selectedProvince}<br />
                      - Selected City: {selectedCity}
                    </div>
                    <div className="col-md-6">
                      <strong>Map State:</strong><br />
                      - City Bounds: {cityBounds ? `${cityBounds[0][0]},${cityBounds[0][1]} to ${cityBounds[1][0]},${cityBounds[1][1]}` : "None"}<br />
                      - Drawing State: {isDrawing ? "Active" : "Inactive"}<br />
                      - Form Enabled: {isFormEnabled ? "Yes" : "No"}<br />
                      - New Asset Data: {newAssetData ? "Ready" : "None"}
                      {newAssetData && (
                        <>
                          <br />- Area: {newAssetData.area?.toFixed(2)} m²
                          <br />- Geometry Type: {newAssetData.geometry?.type || "Array"}
                        </>
                      )}
                    </div>
                  </div>
          ...
          <Row>
            <Col md={7}>
                  ...
                  {isDrawing && (
                    <Alert variant="warning" className="mb-0 mt-2 py-2">
                      <small>
                        ...
                        Klik pada peta untuk mulai menggambar polygon. Klik ganda untuk menyelesaikan.
                      </small>
                    </Alert>
                  )}
                </Card.Header>
              ...
              <div style={{ height: "70vh", width: "100%" }} className="border rounded shadow-sm overflow-hidden">
                <PetaAsetYardip
                  assets={[]}
                  isDrawing={isDrawing}
            ...
            <Col md={5}>
              <div className="card border-0 shadow-sm">
                <FormYardip
                  ...
                  selectedProvince={selectedProvince === 'jateng' ? 'Jawa Tengah' : 'DI Yogyakarta'}
                />
              </div>
            </Col>
          </Row>
